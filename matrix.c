              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        // INSERT YOUR CODE HERE
	// given a vector of source vertices, find the neighbors
        // HINT: spmv 
	...
	// Loop through the rows.
	  ...
	  // Get begin and end index into col and val for row.
          for(int j = 0; j < rows; j++) {
            // Get vector with edges and children by multiplying src by transposed matrix
	    dst[j] += mat_trans[i][j] * src[i];
          }
       	}
