         ...
         int* color, int* distance, int* parent)
{
    ...
    // Initialize the vertices for all but the source
    ...
    parent[source] = -1;
    // color should be initialized to 1 because it's the first vertex in the queue
    color[source] = 1;
    ...
    // initialize first node on queue
    // enqueue(Q,s)
    add_node(queue, 0, create_node(source));
    // while(Q is not empty)
    while(*queue != NULL) { 
      // first in first out queue
      int u = remove_node(&queue[0]);
      // list of neighbors to current node u
      adj_node_t* neighbor = list[u];
      while(neighbor != NULL) {
	// if node not visited
        if(color[neighbor->vid] == 0) {
	    // color adjacent node to u
	    color[neighbor->vid] = 1;
	    // set distance from source
	    distance[neighbor->vid] = distance[u] + 1;
	    // set previous node
	    parent[neighbor->vid] = u;
	    // place node on queue to later process its neighbors
	    add_node(queue, 0, create_node(neighbor->vid));
        }
	// go to next neighbor
